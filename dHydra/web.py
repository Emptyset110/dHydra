...
class WorkerHandler(tornado.web.RequestHandler):
    ...
    def get(self, worker_name, method_name):
        if method_name == "":
            method_name = "index"
        print(
            "Worker Name: {}, method_name: {}"
            .format(worker_name, method_name)
        )
        try:
            self.render(method_name + ".html")
        except Exception as e:
            self.write(
                "Cannot render: " + self.get_template_path() +
                "/" + method_name + ".html"
            )
    ...
    def get_template_path(self):
        ...
        if os.path.exists(
                os.getcwd() + "/Worker/" +
                self.path_args[0] + "/templates/" +
                self.path_args[1] + ".html"
        ):
            return os.getcwd() + "/Worker/" + self.path_args[0] + "/templates"
        else:
            return (
                os.path.split(os.path.realpath(__file__))[0] +
                "/Worker/" + self.path_args[0] + "/templates"
            )
