	...
	def audience_distribution(self, movie=None, name=None, dataframe=False):
		if movie == None:
			...
			if len(movieid)==0:
				...
				print("共找到%d个与\"%s\"相关的电影，只显示第一个"%(len(movieid),name))
				movie = movieid[0]
		...
		for movie in movieid:
			data = self.session.get("http://pf.maoyan.com/movie/viewerStatistic.json?movieId=%s"%movie).json()["data"]
			print(data)
			if dataframe:
				data_df = pandas.DataFrame(data, index = [int(data["movieId"])])
				if result is None:
					result = data_df
				else:
					result = result.append(data_df, ignore_index=True)
			else:
				result.append(data)
			print(result)
		return result
