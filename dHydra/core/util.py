...
def ws_parse(message, to_dict=False):
    data_list = re.findall(
        r'(?:((?:2cn_)?((?:sh|sz)[\d]{6})(?:_0|_1|_orders|_i)?)(?:=)(.*)(?:\n))', message)
    result = list()
    for data in data_list:
        if (len(data[0]) == 12):  # quotation
            ...
            wstype = 'other'
        result = ws_parse_to_list(wstype=wstype, symbol=data[1], data=data[
                                  2], result=result, to_dict=to_dict)
    return result
...
def quotation_to_dict(data):
    ...
    if len(data) == 68:
        quotation = {
            "data_type": 'quotation',	"symbol": data[1]  # "股票代码"
            ,   "name": data[2]  # "中文名"
            # "datetime格式的日期时间"
            # "昨收"
            # "今开"
            ...
            ,   "time": datetime.strptime(data[3] + ' ' + data[4], "%H:%M:%S %Y-%m-%d"),   "last_close": float(data[5]),   "open": float(data[6]),   "high": float(data[7]),   "low": float(data[8]),   "now": float(data[9]),   "status": data[10],   "deal_count": float(data[11])  # "成交笔数"
            ...
            # "买入撤单总金额"?
            # "卖出撤单笔数" ?
            ,   "average_ask": float(data[15]) if data[15] else 0.0,   "current_bid": int(data[16]) if data[16] else 0,   "average_bid": float(data[17]) if data[17] else 0.0,   "cancel_ask_num": int(data[18]),   "cancel_ask_volumn": int(data[19]),   "cancel_ask_amount": float(data[20]),   "cancel_bid": int(data[21]),   "cancel_bid_volumn": int(data[22])  # "卖出撤单量"?
...
def deal_to_dict(data):
    deal = {
        "data_type": 'deal',   "symbol": data[1]   # 股票代码
        ,   "index": data[2]  # 成交序号
